                    ...
                    try:
                        ...
                        if instruction_field == "number_of_read_registers":
                            ...
                            for i in range(int(instruction_field_value)):
                                read_reg = next(line_iter)
                                instruction.update(
                                    {instruction_field + "_" + str(i): read_reg}
                                )
                            ...
                            instruction_field = next(instruction_fields_iter)
                            for i in range(int(instruction_field_value)):
                                write_reg = next(line_iter)
                                instruction.update(
                                    {instruction_field + "_" + str(i): write_reg}
                                )
        ...
        for j in range(memory_instructions_in_block[int(block)]):
            ...
            if stat_trace[int(block)][j]["is_read_flag"] == "1":
                if instruction in memory_instructions_info.keys():
                    ...
                    stride = int(address) - info["last_address_in_access1"]
                    if stride in info["access1_strides"].keys():
                        info["access1_strides"].update(
                            {stride: info["access1_strides"][stride] + 1}
                        )
                    else:
                        info["access1_strides"].update({stride: 1})
                    info.update({"last_address_in_access1": int(address)})
                    info = updateStrideInAccess(
                        info,
                        int(address),
                    )
                    ...
                    if stat_trace[int(block)][j]["is_write_flag"] == "1":
                        ...
                        stride = int(address) - info["last_address_in_access3"]
                        if stride in info["access3_strides"].keys():
                            info["access3_strides"].update(
                                {stride: info["access3_strides"][stride] + 1}
                            )
                        else:
                            info["access3_strides"].update({stride: 1})
                ...
                if instruction in memory_instructions_info.keys():
                    ...
                    stride = int(address) - info["last_address_in_access2"]
                    if stride in info["access2_strides"].keys():
                        info["access2_strides"].update(
                            {stride: info["access2_strides"][stride] + 1}
                        )
                    else:
                        info["access2_strides"].update({stride: 1})
